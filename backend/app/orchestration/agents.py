...
def run_sub_agent(source_type: str, temp_request_id: str, user_text: str, url: str | None) -> dict:
    ...
    try:
        ...
        # --- Executor: fetch external content for this source if a URL was given ---
        if url:
            already_fetched = db.execute(
                select(TempDocument).where(
                    ...
                    TempDocument.external_url == url,
                )
            ).scalar_one_or_none()

            if already_fetched is None:
                fetch_result = call_source_agent(source_type, url)
                doc = TempDocument(
            ...
            # Idempotent (see chunk_embed_service fix) — safe even if other
            # sub-agents already chunked their own docs for this request.
        ...
        # --- Normalize: extract candidate title/specs from the retrieved text ---
        ...
        # --- Confidence Score: retrieval quality only (cross-agent agreement is Phase 7's job) ---
        if retrieved:
